...
if cloudinary_url:
    print(f"✓ CLOUDINARY_URL environment variable found")
    ...
    try:
        ...
        if match:
            ...
            print(f"✓ Parsed CLOUDINARY_URL successfully:")
        ...
        print(f"✗ Error parsing CLOUDINARY_URL: {str(e)}")
        sys.exit(1)
...
try:    # A simple test upload using a placeholder image
    print("Attempting to upload a test image...")
    test_upload = cloudinary.uploader.upload(
        "https://res.cloudinary.com/demo/image/upload/v1312461204/sample.jpg",
        public_id="test-image-{0}".format(cloudinary.config().cloud_name)
    )
    
    if test_upload and 'public_id' in test_upload:
        print(f"✓ Successfully uploaded test image to Cloudinary!")
        print(f"  Public ID: {test_upload['public_id']}")
        print(f"  URL: {test_upload['url']}")
    else:
        print(f"✗ Upload test returned unexpected result")
        sys.exit(1)
except Exception as e:
