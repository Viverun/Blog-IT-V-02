...
def generate_view_context(request, **additional_context):
    """
    Generate a rich context dictionary for AI tools to use.
    
    This function collects information about the current request, user,
    and other relevant data that might be useful for AI processing.
    
    Args:
        request: The Django request object
        **additional_context: Any additional context data to include
    
    Returns:
        dict: A dictionary with request and project context
    """
    context = {
        'request': {
            'path': request.path,
...
def ai_context_processor(request):
    """
    Django context processor that adds AI tool capabilities to template context.
    
    To use this, add it to your TEMPLATES context_processors in settings.py:
    'django_ai_tools.ai_context_processor',
    
    Returns:
        dict: A dictionary with AI helper functions
    """
    return {
        'ai_tools': {
            'get_model_structure': get_model_structure,
            'get_url_patterns': get_url_patterns,
        }
    }